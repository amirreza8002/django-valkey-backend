...
@unittest.skipUnless(ValkeyCache_params, "Valkey backend not configured")
@override_settings(
    CACHES=caches_setting_for_tests(
        base=ValkeyCache_params,
        exclude=valkey_excluded_caches,
    )
)
class ValkeyCacheTests(BaseCacheTests, TestCase):
    def setUp(self):
        import valkey
    ...
    @property
    def incr_decr_write_connection(self):
    ...
    def test_cache_client_class(self):
        self.assertIs(cache._class, ValkeyCacheClient)
        self.assertIsInstance(cache._cache, ValkeyCacheClient)

    def test_get_backend_timeout_method(self):
        positive_number = 10
        positive_backend_timeout = cache.get_backend_timeout(positive_number)
        self.assertEqual(positive_backend_timeout, positive_number)
    ...
    def test_get_client(self):
        self.assertIsInstance(cache._cache.get_client(), self.lib.Valkey)
